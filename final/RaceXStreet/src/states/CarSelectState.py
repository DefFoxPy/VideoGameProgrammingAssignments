...
class CarSelectState(BaseState):
    def enter(self) -> None:
        self.player = Player(
            (settings.VIRTUAL_WIDTH - settings.CAR_WIDTH) // 2,
            (settings.VIRTUAL_HEIGHT - settings.CAR_HEIGHT) // 2,
        )
    ...
    def on_input(self, input_id: str, input_data: InputData) -> None:
        if input_id == "move_right" and input_data.pressed:
            ...
            self.state_machine.change(
                ...
                player=self.player,
                car_list=[],
                datos=[0, 0, [], 0, 0, False, False, 0],
            )

    def render(self, surface: pygame.Surface) -> None:
        if self.displayX < settings.VIRTUAL_WIDTH:
            surface.blit(
                settings.TEXTURES["EscenaStar"].convert_alpha(), [self.displayX, 0]
            )
        elif self.displayX < settings.VIRTUAL_WIDTH * 2:
            surface.blit(
                settings.TEXTURES["EscenaSelect"].convert_alpha(),
                (self.displayX - settings.VIRTUAL_WIDTH * 2, 0),
            )
        else:
            ...
            render_text(
                ...
                font_display,
                settings.VIRTUAL_WIDTH // 2,
                (settings.VIRTUAL_HEIGHT + settings.TEXTURES["cartel1"].get_height())
                // 2
                + 45,
                color_display,
                center=True,
            ...
            surface.blit(
                ...
                (
                    (settings.VIRTUAL_WIDTH) // 2 - 130 - settings.ICON_WIDTH,
                    (settings.VIRTUAL_HEIGHT) // 2 - 40,
                ),
                settings.FRAMES["list_icons"][26],
            )
            if self.player.skin == 0:
                surface.blit(
                    ...
                    (
                        (settings.VIRTUAL_WIDTH) // 2 - 130 - settings.ICON_WIDTH,
                        (settings.VIRTUAL_HEIGHT) // 2 - 45,
                    ),
                    settings.FRAMES["list_icons"][2],
                )
            ...
            surface.blit(
                settings.TEXTURES["icons"].convert_alpha(),
                (
                    (settings.VIRTUAL_WIDTH) // 2 + 130,
                    (settings.VIRTUAL_HEIGHT) // 2 - 40,
                ),
                settings.FRAMES["list_icons"][27],
            )
