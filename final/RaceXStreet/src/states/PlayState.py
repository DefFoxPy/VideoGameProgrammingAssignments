...
class PlayState(BaseState):
    ...
    def update(self, dt: float) -> None:
        if self.animation == 0:
            ...
            if not self.player.slowly:
                settings.PLAYER_SPEED = min(
                    450, settings.PLAYER_SPEED + self.score // 100 // 2
                )
            ...
            if self.time_car >= settings.GENERATE_CAR + random.randint(
                1, max(1, 10 - self.score // 100)
            ):
            ...
            if self.start_timer == 11:
                self.state_machine.change(
                    "enterHighScore", score=self.score / 100, world=self.world
                )
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        surface.blit(
            settings.TEXTURES["icons"].convert_alpha(),
            (1020 + settings.ICON_WIDTH, 10),
            settings.FRAMES["list_icons"][18],
        )
        ...
        surface.blit(
            settings.TEXTURES["icons"].convert_alpha(),
            (60 + settings.ICON_WIDTH, settings.VIRTUAL_HEIGHT - 140),
            settings.FRAMES["list_icons"][1],
        )
        ...
        surface.blit(
            settings.TEXTURES["icons"].convert_alpha(),
            (30, settings.VIRTUAL_HEIGHT - 140),
            settings.FRAMES["list_icons"][0],
        )
        if self.powerUpSlowly:
            surface.blit(
                settings.TEXTURES["icons"].convert_alpha(),
                (30, settings.VIRTUAL_HEIGHT - 140),
                settings.FRAMES["list_icons"][24],
            )
        ...
        if self.animation == 3:
            surface.blit(
                settings.TEXTURES["explosion"].convert_alpha(),
                (self.player.x - 27, self.player.y - 15),
                settings.FRAMES["explosion_animation"][self.start_timer],
            )
            surface.blit(
                settings.TEXTURES["explosion"].convert_alpha(),
                (self.position_carX, self.position_carY),
                settings.FRAMES["explosion_animation"][self.start_timer],
            )
        pygame.display.flip()
        ...
        if input_id == "pause":
            ...
            self.state_machine.change(
                ...
                datos=[
                    ...
                    self.time_car,
                    self.old_skin_car,
                    self.powerUp_limit_ghost,
                    self.powerUp_limit_slowly,
                    self.powerUpGhost,
                    self.powerUpSlowly,
                    self.animation,
                ],
